/**
 * Equation Node Design
 * Dashboard-style layout with title, equation (MathJax), and metadata sections
...
function renderSVG(
  ...
  const typeFontSize = params.typeFontSize ?? 11;
  ...
  // Draws: start at (0, r), curve to (r, 0), line to (w-r, 0), curve to (w, r), 
  //        line down to (w, topHeight), line left to (0, topHeight), close
    ...
    <svg width="${svgWidth}" height="${svgHeight}" xmlns="http://www.w3.org/2000/svg">
      ...
      <g filter="url(#${shadowId})">
        <g ${filter}>
          ...
          <!-- Bottom Section: Metadata (rounded bottom corners only) -->
          <path d="${bottomPath}" fill="${bottomColor}" opacity="${bgAltOpacity}"/>
        </g>
      </g>
...
/**
 * Get Cytoscape style for equation node
 */
export async function getEquationNodeStyle(
  node: Node,
  params: EquationNodeParams,
  theme: ColorTheme
