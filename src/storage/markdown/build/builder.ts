...
import { normalizeEdgeTypeName, type BuildSelection } from './selection-dialog';
...
export function buildGraphFromDocument(parsed: KnograDocument, selection: BuildSelection): BuiltGraph {
  const now = new Date();
  ...
  const nodes = parsed.nodes.map((node, index): Node => {
    ...
    return {
      ...
      properties: {
        ...(equation ? { equation } : {}),
        ...(comment ? { comment } : {}),
        // The id this node was known by in the source document, so the same
        // document can later be re-run as an Update (markdown-architecture §6.3).
        externalId: node.mermaidId,
      },
      createdAt: now,
      updatedAt: now,
      attachments: [],
  ...
  const buildScene = (
    ...
    sceneId: SceneId,
    title: string,
    layoutMode: BuildSelection['layout'],
  ...
  if (selection.importNotes) {
    // `Knogra notes` produces real notes — `source: 'note'`, the same shape the
    // note editor writes — so the section round-trips. Only the legacy
    // `Knogra tutorial` section still produces `'tutorial'`, which exports into
    // the ai-chat section and is never read back (markdown-architecture §5.1).
    const messagesByNodeId = new Map<NodeId, ChatMessage[]>();
    let sequence = 0;

    const addNote = (note: DocumentNote, source: MessageSource): void => {
      const nodeId = idByMermaidId.get(note.nodeId);
      if (!nodeId) return;
      const messages = messagesByNodeId.get(nodeId) ?? [];
      messages.push({
        id: `msg-${prefix}-${sequence++}`,
        role: source === 'note' ? 'user' : 'assistant',
        content: note.content,
        timestamp: now,
        source,
        // Carrying the document's own note id is what lets a re-run replace this
        // note instead of adding a second copy.
        ...(note.noteId ? { externalId: note.noteId } : {}),
      });
      messagesByNodeId.set(nodeId, messages);
    };

    for (const note of parsed.notes) addNote(note, 'note');
    for (const note of parsed.articles) addNote(note, 'tutorial');

    // Node order, so conversations land in the same sequence as the diagram.
    for (const node of parsed.nodes) {
      const nodeId = idByMermaidId.get(node.mermaidId);
      const messages = nodeId ? messagesByNodeId.get(nodeId) : undefined;
      if (!nodeId || !messages) continue;
      conversations.push({ nodeId, messages, createdAt: now, updatedAt: now });
    }
  }
...
function selectSceneCentralIds(parsed: KnograDocument, selection: BuildSelection): string[] {
...
function createEdgeTypeImportPlan(selection: BuildSelection): {
  edgeTypes: EdgeType[];
