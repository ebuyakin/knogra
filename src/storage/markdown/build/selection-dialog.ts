...
import type { KnograDocument } from '../document/document';
import {
  getCommentStatus,
  getEquationStatus,
  getNoteStatus,
...
import { getMermaidImportLayoutSettings, type MermaidImportLayoutSettings } from './layout-settings-store';
import { showMermaidLayoutOptionsDialog, layoutHasOptions } from './layout-options-dialog';
...
export function showBuildSelectionDialog(
  parsed: KnograDocument
): Promise<BuildSelection | null> {
  ...
  const equationMetadataStatus = getEquationStatus(parsed);
  const tagMetadataStatus = getTagStatus(parsed);
  const noteMetadataStatus = getNoteStatus(parsed);
  const commentMetadataStatus = getCommentStatus(parsed);
  ...
  return new Promise(resolve => {
    ...
    const syncLayoutOptionsButton = (): void => {
      const enabled = layoutHasOptions(layoutSelect.value as BuildSelection['layout']);
    ...
    layoutOptionsButton.addEventListener('click', () => {
      const layout = layoutSelect.value as BuildSelection['layout'];
    ...
    const noteCb = createMetaImportCheckbox('Notes / articles', noteMetadataStatus);
    ...
    const close = (value: BuildSelection | null): void => {
      overlay.remove();
      resolve(value);
    };
    ...
    importButton.addEventListener('click', () => {
      ...
      close({
        ...
        layout: layoutSelect.value as BuildSelection['layout'],
        ...
        sceneGeneration: sceneGenSelect.value as BuildSelection['sceneGeneration'],
...
function countHubNodes(parsed: KnograDocument): number {
...
function getEdgeLabelRows(parsed: KnograDocument): EdgeLabelRow[] {
