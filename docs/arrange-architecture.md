...
> **Authority:** Canonical for the **Arrange feature** (`src/features/arrange/`): the selection-scoped geometric tools, their contract, the tool registry, and the shared execution pipeline. The domain terminology that distinguishes Arrange from Auto-layout is defined in [layout-architecture.md](layout-architecture.md) §1.1 and is not repeated here.
...
interface ArrangeTool {
  ...
  label: string;                   // UI label, without the shortcut hint
  shortcut?: string;               // omitted for menu-only tools
  group: ArrangeGroup;             // 'align' | 'distribute' | 'shape' | 'spacing'
...
7. **Animation** — `NodePositionAnimator`, a 200 ms glide, positions only (no `ViewportTarget`, per invariant 4).
8. **Persistence** — `graphSaver.suspend('arrange')` across the glide, then one `forceSave`.
...
- **Same tool repeated** — a `repeats` counter accumulates. Folded in as `spacingStep ** repeats`, so N presses of Spread compose to exactly N steps with **no drift**. This is the only knob that composes multiplicatively; tools without one ignore `repeats`, and re-running them is idempotent anyway.
...
The **Arrange nodes** submenu is built once, in `ContextMenu.#buildArrangeMenu`, and used by both the node menu and the canvas menu. It iterates `arrange.tools()` and:

- renders `label (shortcut)`, or just `label` for menu-only tools;
- inserts a divider whenever `group` changes — which is why the `TOOLS` literal keeps groups contiguous;
- enables each entry only at `selectionSize >= tool.minNodes`, and the whole submenu at `minimumSelection()`.

Selection size comes from `arrange.selectionSize()` rather than the UI counting nodes itself, so the menu's notion of "selected" matches the feature's (`:selected:visible`) exactly.
...
*Why Grid exists when Circle already squares four nodes:* Circle produces a **rotated** square, because it minimises travel. What makes a diagram read as deliberate is axis alignment, which is Grid's entire contribution.
...
- **More tools:** arc, mirror/flip across the selection's axis, selection-scoped rotate (deferred as low value once Circle landed).
- **Keyboard namespace.** After Grid the global keymap has only `Shift+N` and digits `5`–`9` free. The agreed escape hatch is an **Arrange leader key** — one key entering a transient sub-mode with an on-screen hint bar. Deliberately unbuilt: it introduces modality and touches the already-oversized keyboard handler.
- **Menu nesting.** The submenu is flat with dividers at nine tools. If the family grows much further, switch to one submenu per group — the registry already carries `group`, so this is a change to `#buildArrangeMenu` alone.
- **Composable params.** If a second multiplicatively-composing knob appears, the merge rule in §4.1 should move onto the tool (an optional `mergeParams`) rather than growing inside `Arrange`.
