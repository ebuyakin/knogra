...
`.github/project-journal.md` is the official append-only record of project work, decisions, fixes, and remaining tasks. Read the newest relevant journal entries first, using workstream/tags once the journal format is updated. `todo.md` is the user-maintained active checklist and should checked by user, not you. At checkpoint/session close, update the journal (but not todo). 
